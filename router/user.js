...
router.post('/register', async (req, res) =>{
    try{
       //mengambil data dari body permintaan(postman / website)
        ...
        userInsert.password = await Bcrypt.hash(password, salt);
    ...
    // update data user
    router.put('/:id', async (req, res) => { //TODO tambahkan ke tiap model/schema lain
        const users = await User.findByIdAndUpdate(req.params.id, req.body,{}) //TODO ubah User menjadi tipe data sesuai model yang lain
        res.status(200).send({
            'status': true,
            'data': users
        });
    ...
    router.delete('/:id', async (req, res) => { //TODO tambahkan ke tiap model/schema lain
        const users = await User.findByIdAndDelete(req.params.id, req.body) //TODO ubah User menjadi tipe data sesuai model yang lain
        res.status(200).send({
            'status': true,
            'data': users
        });
